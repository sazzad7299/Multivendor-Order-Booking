        ...
        <header class="py-4 bg-gray-700">
            <div class="container flex items-center justify-between px-6 mx-auto">
                ...
                <nav class="space-x-4 text-sm text-gray-300 sm:text-base">
                    @guest('developer')
                        <a class="no-underline hover:underline" href="{{ route('developer.login') }}">{{ __('Login') }}</a>
                        @if (Route::has('register'))
                            <a class="no-underline hover:underline" href="{{ route('developer.register') }}">{{ __('Register') }}</a>
                        @endif
                    @else
